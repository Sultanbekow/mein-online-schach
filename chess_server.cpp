...
int main() {
    ...
    // 2. Port freigeben (verhindert "Address already in use"-Fehler)
    ...
    address.sin_addr.s_addr = INADDR_ANY; // Auf allen Netzwerkschnittstellen lauschen
    ...
    std::cout << "[SERVER] Schach-Server gestartet auf Port " << PORT << "...\n";
    ...
    std::cout << "[SERVER] Spieler 1 verbunden!\n";
    send(player1_fd, "COLOR:W", 7, 0); // Dem Client mitteilen, dass er Weiß ist
    ...
    std::cout << "[SERVER] Spieler 2 verbunden!\n";
    send(player2_fd, "COLOR:B", 7, 0); // Dem Client mitteilen, dass er Schwarz ist

    std::cout << "[SERVER] Beide Spieler bereit. Spiel startet!\n";

    // 5. Hauptschleife: Daten hin- und herleiten
    while (true) {
        // Nachricht von Spieler 1 empfangen und an Spieler 2 senden
        std::memset(buffer, 0, BUFFER_SIZE);
        int valread = recv(player1_fd, buffer, BUFFER_SIZE, 0);
        if (valread <= 0) {
            std::cout << "[SERVER] Spieler 1 hat die Verbindung getrennt.\n";
            break;
        }
        std::cout << "[MOVE] Spieler 1 -> Spieler 2: " << buffer << "\n";
        ...
        if (valread <= 0) {
            std::cout << "[SERVER] Spieler 2 hat die Verbindung getrennt.\n";
            break;
        }
        std::cout << "[MOVE] Spieler 2 -> Spieler 1: " << buffer << "\n";
